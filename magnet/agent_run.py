...
def _log_path(ledger_path: str | None, root: str) -> str:
...
def run_deterministic_loop(
    *, ledger_path: str | None = None, repo_root: str | None = None, probe_name: str = DEMO_PROBE
) -> str:
    root = repo_root or os.getcwd()
    path = _log_path(ledger_path, root)
    conn = connect(path)
    reset_demo(conn)
    ...
    adoption = tool_adopt_change(
        ...
        probe_name, ledger_path=path, apply_demo_bonus=True,
    )
    steps.append(f"  [adopt_change]  id={adoption['id']}  {adoption['description']}")

    rec2 = tool_record_week(probe_name, ledger_path=path, simulate_next_week=True)
    steps.append(
...
def run_strands_agent(
    *,
    ...
    agent = create_agent(model=model, ledger_path=path, repo_root=root)
    ...
    *,
    ...
    if mode == "none":
        return run_deterministic_loop(
            ledger_path=ledger_path, repo_root=repo_root, probe_name=probe_name
        )

    try:
        return run_strands_agent(
            ledger_path=ledger_path, repo_root=repo_root, probe_name=probe_name, mode=mode
        )
