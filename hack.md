...
- 2026-09-04 · **WAVE Bedrock live-or-BLOCKED:** cloud STS `NoCredentialsError` · exact missing `AWS_ACCESS_KEY_ID` `AWS_SECRET_ACCESS_KEY` `AWS_REGION` · IMDS/alt creds absent · `magnet agent-run --model bedrock` → DEGRADED.
...
- 2026-09-04 · Judge/stranger paths re-run: `bash scripts/judge-demo.sh` → JUDGE DEMO OK · `bash scripts/stranger-pass.sh` → stranger pass OK · `python3 -m pytest -q` → 118 passed · check-docs 11 PASS · model id re-derived `global.anthropic.claude-sonnet-4-6` (strands-agents 1.54.0) · no Devpost submit.
