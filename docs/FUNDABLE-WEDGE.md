...
| **Apply** | Write the skill into the measured stack so coverage can move — or refuse helped when it did not | `magnet stack-demo` · `magnet adopt --apply` |
...
2. **Embarrassment hunt built in** — `magnet eval`, `magnet bakeoff`, and `magnet stack-demo` ship arms that can beat us (silent_null, naive_stars, naive-helped-on-noise).
