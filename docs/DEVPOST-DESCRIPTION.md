...
112 rather than 113 because one integration test is marked `slow` and the advertised probe command deselects it.
...
- `pytest` is in the `[dev]` extra, not in the base install. A fresh clone that runs `pip install -e .` and then `pytest` has no pytest in its virtualenv: on a clean machine the command is not found, and on a machine with a system pytest on the path the tests that spawn the verification scripts fail. `pip install -e ".[dev]"` first, then the suite is green: 131 passed (command: `python3 -m pytest -q`, 2026-09-03, fresh clone in a Python 3.12.5 virtualenv).
- If pytest is missing, `magnet probe pytest-pass-rate` used to print `pytest-pass-rate: None` and exit 0. **Fixed 2026-09-05 (Slice 16):** `magnet probe` exits 1 when value is None and prints `unmeasured`.
