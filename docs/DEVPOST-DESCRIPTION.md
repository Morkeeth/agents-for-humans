...
- `pytest` is in the `[dev]` extra, not in the base install. A fresh clone that runs `pip install -e .` and then `pytest` has no pytest in its virtualenv: on a clean machine the command is not found, and on a machine with a system pytest on the path the tests that spawn the verification scripts fail. `pip install -e ".[dev]"` first, then the suite is green: 309 passed (command: `python3 -m pytest -q`, 2026-09-12; count re-derived from `tests/test_*.py`).
