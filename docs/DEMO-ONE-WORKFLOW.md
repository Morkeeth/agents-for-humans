...
> Current numbers (re-derived 2026-09-14 via `bash scripts/one-workflow.sh`): `254/254 → 253/254 hurt → 254/254 helped`, prediction `pass rate recovers by 1` → **prediction-held**. Transcript: `docs/screenshots/one-workflow.txt`.
*For the Sep 14 video. One workflow, nothing else on screen. Scripted path: `bash scripts/one-workflow.sh` (exit 0 only when recover grades as rise). Every line in the sidecar is pasted from a live run.*

**Your eval** is the repo's test suite. It includes `tests/test_prompt_contract.py`, two lines that pin the one rule MAGNET's system prompt must carry. **The prompt** is `SYSTEM_PROMPT` in `magnet/tools.py`. Drop the rule, the eval drops by one, MAGNET says `hurt`. Put it back, MAGNET says `helped`. Nothing is simulated; `--no-simulate` on every call. The restore prediction is **`pass rate recovers by 1`** — not a synonym. Slice 32 made `recover`/`restore` rise intent so that prediction grades.
