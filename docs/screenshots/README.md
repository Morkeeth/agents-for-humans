...
Sidecars without a PNG: `list-probes.txt`, `check-docs.txt` (11 claims, all match), `drift-demo.txt` (fake repo 2 drifts exit 1, this repo 0 drifts exit 0), `probe-pytest-pass-rate.txt` (173/173; one test marked `slow` is deselected by the probe command), `pytest.txt` (174 passed, full suite, `python3 -m pytest -q`).
