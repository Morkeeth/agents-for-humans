...
|---|---|---|
| 1 required | `one-workflow.png` | Change one prompt, MAGNET re-runs your eval: 173/173 baseline, 111/112 hurt (-1), 173/173 helped (+1), 21 s wall clock, nothing simulated |
...
Sidecars without a PNG: `list-probes.txt`, `check-docs.txt` (11 claims, all match), `drift-demo.txt` (fake repo 2 drifts exit 1, this repo 0 drifts exit 0), `probe-pytest-pass-rate.txt` (189/189; one test marked `slow` is deselected by the probe command), `pytest.txt` (190 passed, full suite, `python3 -m pytest -q`).
