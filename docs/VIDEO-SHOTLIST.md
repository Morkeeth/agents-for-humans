...
| 3 | 0:35 to 0:50 | `pytest-pass-rate: 112/112` then `recorded pytest-pass-rate: verdict=baseline readings=1` | `python3 -m magnet.cli probe pytest-pass-rate` then `$M record pytest-pass-rate` | "My eval is this repo's test suite: 112 of 112. Record it as the baseline. One reading, so MAGNET says baseline. It will not call a trend from one number." |
